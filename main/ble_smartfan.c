...
#define GATTS_NUM_HANDLE_TEST_A     4  // số lượng handle (Service,Characteristic, Characteristic value, Characteristic descriptor)
//////////////////////

////////////////////// service 2  nut nhan che do, on off
...
//////////////////////// service 3 timer
#define GATTS_SERVICE_UUID_TEST_C   0x180A//0x00FF
#define GATTS_CHAR_UUID_TEST_C      0x2A14//0xFF01
#define GATTS_DESCR_UUID_TEST_C     0x1111
#define GATTS_NUM_HANDLE_TEST_C    4

//////////////////////// service 4 slide
#define GATTS_SERVICE_UUID_TEST_D   0x180C
#define GATTS_CHAR_UUID_TEST_D      0x2A16
#define GATTS_DESCR_UUID_TEST_D     0x4444
#define GATTS_NUM_HANDLE_TEST_D    4

////////////////////// khai bao led va cac nut
#define led_1 13
#define led_2 12
#define led_3 14
#define led_a 27
...
#define TEST_DEVICE_NAME            "ESP_BLE_Smart_Fan"
...
static uint8_t char1_str[] = {0x11,0x22,0x33,0x44};
...
static void inline test(uint64_t counter_value)
{
...
//adv data
static esp_ble_adv_data_t adv_data = {// cấu hình dữ liệu quảng cáo
    .set_scan_rsp = false,
    .include_name = true,
...
static esp_ble_adv_params_t adv_params = {//thông số quảng cáo 
    .adv_int_min        = 0x20,
    .adv_int_max        = 0x40,
...
struct gatts_profile_inst {/////// cấu trúc profile gatt
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT:
        ...
        break;
#endif
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT: // kiểm tra trạng thái quảng cáo
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {////////////////////// DOC gia tri value
        ...
        rsp.attr_value.value[0] = DHT11_read().temperature-5;// gui gia tri cam bien dht11
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ESP_LOGI("adsf", "GATT_WRITE_EVT, value len %d, value :", param->write.len);
            esp_log_buffer_hex("du lieu ", param->write.value,1);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ESP_LOGI("ma hex b", "GATT_WRITE_EVT, conn_id %d, trans_id %d, handle %d\n", param->write.conn_id, param->write.trans_id, param->write.handle);
        if (!param->write.is_prep){
            ESP_LOGI("ma hex b", "GATT_WRITE_EVT, value len %d, value :", param->write.len);
            esp_log_buffer_hex("ma hex b1", param->write.value,param->write.len);
//////////////////////////////////////////////////////////////////////////////////////////////////////
// bat tat io
        uint16_t descr_value= param->write.value[1]<<8 | param->write.value[0];
...
static void gatts_profile_c_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, conn_id %d, trans_id %d, handle %d", param->write.conn_id, param->write.trans_id, param->write.handle);
        if (!param->write.is_prep){
            ESP_LOGI("abc", "GATT_WRITE_EVT, value len %d, value :", param->write.len);
            esp_log_buffer_hex("du lieu ", param->write.value,1);
                uint16_t descr_value= param->write.value[1]<<8 | param->write.value[0];
...
static void gatts_profile_d_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ESP_LOGI("adsf", "GATT_WRITE_EVT, value len %d, value :", param->write.len);
            esp_log_buffer_hex("du lieu D ", param->write.value,1);
 ///////////////////////////////////////////////////////////////   che do slide       
            uint16_t descr_value= param->write.value[1]<<8 | param->write.value[0];
                if(descr_value ==0x00) pwm=0;
                if(descr_value ==0x01) pwm=888;
                if(descr_value ==0x02) pwm=1777;
                if(descr_value ==0x03) {pwm=2664; bat_led(1);}
                if(descr_value ==0x04) pwm=3552;
                if(descr_value ==0x05) pwm=4440;
                if(descr_value ==0x06) {pwm=5328; bat_led(2);} 
                if(descr_value ==0x07) pwm=6216;
                if(descr_value ==0x08) pwm=7104;
                if(descr_value ==0x09) {pwm=8000; bat_led(3);} 
              ESP_ERROR_CHECK(ledc_set_duty(LEDC_MODE, LEDC_CHANNEL,pwm));
              ESP_ERROR_CHECK(ledc_update_duty(LEDC_MODE, LEDC_CHANNEL)); 
              gpio_set_level(nut1, 1);
              at=0;
...
void control_panel(void *pvParameter)
{
     gpio_pad_select_gpio(led_1);
     gpio_pad_select_gpio(led_2);
     gpio_pad_select_gpio(led_3);
     gpio_pad_select_gpio(led_a);
     gpio_pad_select_gpio(led_t);
     gpio_pad_select_gpio(but_o);
     gpio_pad_select_gpio(but_1);
     gpio_pad_select_gpio(but_2);
     gpio_pad_select_gpio(but_3);
     gpio_pad_select_gpio(but_a);

     gpio_set_direction(led_1, GPIO_MODE_OUTPUT);
     gpio_set_direction(led_2, GPIO_MODE_OUTPUT);
     gpio_set_direction(led_3, GPIO_MODE_OUTPUT);
     gpio_set_direction(led_a, GPIO_MODE_OUTPUT);
     gpio_set_direction(led_t, GPIO_MODE_OUTPUT);

     gpio_set_direction(but_o, GPIO_MODE_INPUT);
     gpio_set_direction(but_1, GPIO_MODE_INPUT);
     gpio_set_direction(but_2, GPIO_MODE_INPUT);
     gpio_set_direction(but_3, GPIO_MODE_INPUT);
     gpio_set_direction(but_a, GPIO_MODE_INPUT);

     gpio_set_pull_mode(but_o, GPIO_PULLUP_ONLY);
     gpio_set_pull_mode(but_1, GPIO_PULLUP_ONLY);
     gpio_set_pull_mode(but_2, GPIO_PULLUP_ONLY);
     gpio_set_pull_mode(but_3, GPIO_PULLUP_ONLY);
     gpio_set_pull_mode(but_a, GPIO_PULLUP_ONLY);
...
void main_task(void *pvParameter)
{
    ...
    example_ledc_init();
    gpio_pad_select_gpio(nut1);
    gpio_set_direction(nut1, GPIO_MODE_OUTPUT);
    ...
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ...
        ret = nvs_flash_init();//cho phép lưu các cặp khóa-giá trị trong bộ nhớ
                               // flash và được một số thành phần như thư viện Wi-Fi sử dụng để lưu SSID và mật khẩu
    }
    ESP_ERROR_CHECK( ret );

    ESP_ERROR_CHECK(esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT));

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();//khởi tạo bộ điều khiển BT 
    ret = esp_bt_controller_init(&bt_cfg);///////////////////////////////////   
    ret = esp_bt_controller_enable(ESP_BT_MODE_BLE);// bật ở Chế độ BLE  
    ret = esp_bluedroid_init();//định nghĩa và API chung cho cả BT Classic  
    ret = esp_bluedroid_enable();//định nghĩa và API chung cho cả BT Classic và BLE
    ret = esp_ble_gatts_register_callback(gatts_event_handler);//xử lý sự kiện  GATT
    ret = esp_ble_gap_register_callback(gap_event_handler);//xử lý sự kiện GAP
    ret = esp_ble_gatts_app_register(PROFILE_A_APP_ID);// ID profiler
    ret = esp_ble_gatts_app_register(PROFILE_B_APP_ID);
    ret = esp_ble_gatts_app_register(PROFILE_C_APP_ID);  
    ret = esp_ble_gatts_app_register(PROFILE_D_APP_ID);     
    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(500); // cấu hình kích thước mtu
///////////// xu li timer dem
...
void app_main(void)
{   
  xTaskCreate(&control_panel, "control_panel_task", 2048, NULL, 1, NULL);
